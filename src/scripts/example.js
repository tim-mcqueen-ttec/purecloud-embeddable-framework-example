  ...
  window.addEventListener('message', function (event) {
    ...
    if (message) {
      if (message.type == 'screenPop') {
        windowProxy = this.window.open(
          `https://example.com?conversationId=${message.data.interactionId.id}&ani=${message.data.interactionId.ani}`,
          '_blank'
          //   message.data.interactionId.id,
          //   'width=600,height=400'
        );
        this.setTimeout(() => this.window.focus(), 100);
        document.getElementById('screenPopPayload').value = event.data;
      } else if (message.type == 'processCallLog') {
        var message = JSON.parse(event.data);

        console.log('processCallLog: BEFORE', message);
        ...
        console.log(
          ...
          message.eventName == 'interactionDisconnected'
        );

        if (message.data.eventName == 'interactionDisconnected') {
          console.debug('processCallLog', message);
          console.log('closing window: ', windowProxy);
          windowProxy.close();
        }
        console.log('processCallLog: AFTER', message);
